...
def index(path):
    ...
    if path.is_dir():
        ...
        aj.save_json_listing(listing, path / 'listing.json')
        aj.save_json_tree(tree, path / 'tree.json')
        aj.save_json_forbidden(forbidden, path / 'forbidden.json')
    else:
...
def duplicate(path, save_listing=False):
    ...
    if path.name == 'listing.json':
        ...
        folder_path = path
        if save_listing:
            aj.save_json_listing(listing, folder_path / 'listing.json')
            aj.save_json_tree(tree, folder_path / 'tree.json')
            aj.save_json_forbidden(forbidden, folder_path / 'forbidden.json')
    else:
        print(f'This is not a valid path - exiting')
        return
    ...
    if duplicate_listing:
        ...
        aj.save_json_listing(duplicate_listing, folder_path / 'duplicate_listing.json')
        if size_gain < 1E3:
...
def missing(old_path, new_path, save_listing=False):
    ...
    if old_path.name == 'listing.json':
        ...
        if save_listing:
            aj.save_json_listing(old_listing, old_folder_path / 'listing.json')
            aj.save_json_tree(old_tree, old_folder_path / 'tree.json')
            aj.save_json_forbidden(old_forbidden, old_folder_path / 'forbidden.json')
    else:
    ...
    if new_path.name == 'listing.json':
        ...
        if save_listing:
            aj.save_json_listing(new_listing, new_folder_path / 'listing.json')
            aj.save_json_tree(new_tree, new_folder_path / 'tree.json')
            aj.save_json_forbidden(new_forbidden, new_folder_path / 'forbidden.json')
    else:
    ...
    if missing_listing:
        aj.save_json_listing(missing_listing, new_folder_path / 'missing_listing.json')
